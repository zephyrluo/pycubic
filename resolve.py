...
def run(plane, adjustFn):
    ...
    # print('param', param)
    try:
        result = subprocess.run(f".\\a.exe {param}", shell=True, capture_output=True, text=True)
        retcmd = result.stdout.strip()
        outstat = result.returncode
        # print("outstat:", outstat, "retcmd:", retcmd)
    except Exception as e:
        print("Error running a.exe:", e)
        retcmd = ""
        outstat = 1
...
def run0(plane, adjustFn):
    ...
    for rotlen in range(1, 6):
        for r in range(12**rotlen):
